...
public class Continuous implements IConstraint {
	...
	public Continuous(List<JuryInfo> listJuryInfos,
			List<Professors> listProfessors, VarInt[] varSlots) {
	...
	private void writeDataCheck(int slotId, int val, String fileName) {
		...
		// Date date = new Date();
		// Common.addToFile("propagate sel_i=" + slotId + ", sel_sl=" + val,
		// fileName);
		// for (Professor p : listProfessors) {
		// ProfessorDuration pd = timeMap.get(p.getID());
		// if (pd != null) {
		// pd.sortTime();
		// Common.addToFile(pd.toString() + " ------ " +
		// calculateViolation(pd.getListTimes()), fileName);
		// }
		// }
