...
class RoomSlot{
	...
	public int getRoom(){ return _r;}
	public int getWeight(){ return _w;}
}
public class AssignRooms {
	...
	private int computeCommon(JuryInfo j1, JuryInfo j2){
		int c = 0;
		if(j2.juryProfessor(j1.getPresidentId())) c++;
		if(j2.juryProfessor(j1.getExaminerId1())) c++;
		if(j2.juryProfessor(j1.getExaminerId2())) c++;
		if(j2.juryProfessor(j1.getSecretaryId())) c++;
		if(j2.juryProfessor(j1.getAdditionalMemberId())) c++;
		
		return c;
	}
	private RoomSlot findRoom(HashSet<Integer> R, Map<Integer, Set<Integer> > S, JuryInfo s, JuryInfo[] listJuryInfos){
		int sel_r = -1;
		int maxCommon = -1;
		...
		while(it.hasNext()){			
			...
			while(irs.hasNext()){
				...
				if(c > maxCommon_r) maxCommon_r = c;
			}
			// maxCommon_r is the maximal number of members in common between jury s and room r
	...
	public void assignRooms(List<JuryInfo> listJuryInfos, int nbRooms){
		...
		for(int i = 0; i < sz-1; i++){
			for(int j = i+1; j < sz; j++){
				if(ji[i].getSlotId() > ji[j].getSlotId()){
					JuryInfo tmp = ji[i]; ji[i] = ji[j]; ji[j] = tmp;
				}
			}
		}
		...
		for(int i = 0; i < sz; i++){
			...
			if(minSl > sl) minSl = sl;
			if(maxSl < sl) maxSl = sl;
		}
		...
		int[][] room_session = new int[nbRooms][maxSl];// room_session[r][i] = the session located at i_th of room r
		int szR[] = new int[nbRooms];// szR[r] is the number of sessions assigned to room r
...
//			System.out.println("AssignRooms::assigmRoom, consider sl_id = " + sl_id + ", session_id = " + session_id + " info : " + ji[session_id].info());
			for(int r = 0; r < nbRooms; r++){
...
//					System.out.println(ji[j].info());
			...
			while(Sid.size() > 0){
				...
				while(is.hasNext()){
					...
					System.out.println("R.size = " + R.size() + ", Sid.size = " + Sid.size() + " try s = " + s + " maxCommon = " + maxCommon + " rs=(" + rs.getRoom() + "," + rs.getWeight());
				}
//				System.out.println(ji[sel_session].info() + " --> FindRoom = " + sel_r);
				R.remove(sel_r);
		...
		for(int r = 0; r < nbRooms; r++){
			...
			for(int s = 0;  s < sz; s++){
				if(var_rooms[s] == r){
					System.out.println(ji[s].getSlotId() + " : " + ji[s].getPresidentName() + "\t" + ji[s].getExaminerName1() + "\t" + 
				ji[s].getExaminerName2() + "\t" + ji[s].getSecretaryName() + "\t" + ji[s].getAdditionalmemberName());
				}
			}
		}
	}
